...
- MediaPipe Face Mesh for face-centered grid tracking
- Grid construction and hit tracking
"""
...
class FaceGridTracker:
    ...
    def __init__(self, grid_width: int = 12, grid_height: int = 15, grid_depth: int = 7):
        """
        ...
        Args:
            ...
            grid_depth: Number of grid points in depth (z-axis, default: 7)
        """
...
class HandCapture:
    ...
    def process_frame(
        ...
        Args:
            frame: BGR frame from webcam
            track_grid_hits: If True, update hit grid with hand landmarks
            draw_grid: If True, draw the face-centered grid on the frame
            show_hits: If True, highlight hit points in the grid visualization
        ...
        # Process face for grid (if drawing grid)
        if draw_grid:
            self.face_grid_tracker.process_frame(frame)
    ...
    def release(self):
        ...
        if self.face_grid_tracker is not None:
            self.face_grid_tracker.release()
